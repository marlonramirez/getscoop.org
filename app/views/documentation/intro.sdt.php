...
<h3>🔐 New: Security Module</h3>

<h4>Encryption System</h4>
<ul>
    <li><b>Cipher with Versioned Crypto</b> - First PHP framework with algorithm versioning</li>
    <li><b>AES-256-GCM</b> encryption (PHP 7.1+)</li>
    <li><b>AES-256-CBC</b> encryption (PHP 5.4+)</li>
    <li>Seamless algorithm migration without re-encrypting database</li>
    <li><b>PBKDF2</b> key derivation (20,000 iterations)</li>
    <li>Multiple encodings: base64, hex, raw</li>
    <li>Automatic encrypted string detection</li>
</ul>

<h4>Security Middlewares</h4>
<ul>
    <li><b>CSRF Guard</b> - Timing-safe token validation
        <ul>
            <li>Protection against Cross-Site Request Forgery</li>
            <li>OpenSSL-based token generation with fallback (PHP 5.4+)</li>
            <li>Automatic <code>@csrf</code> directive in templates</li>
            <li>Validates from header, query, or body</li>
            <li>Session-persistent tokens</li>
        </ul>
    </li>
    <li><b>CORS Guard</b> - Complete Cross-Origin Resource Sharing
        <ul>
            <li>Wildcard subdomain support (<code>*.example.com</code>)</li>
            <li>Preflight request handling</li>
            <li>Configurable origins, methods, headers</li>
        </ul>
    </li>
    <li><b>PSR-15 Compliant</b> - Standard middleware interface</li>
</ul>

<h3>🏗️ Improved: Persistence Architecture</h3>

<h4>Dependency Injection Pattern</h4>
<ul>
    <li><b>Connection-First Architecture</b> - Builders now receive Connection as first parameter
        <ul>
            <li><code>new Builder\Creator($connection, $query, $values, $fields)</code></li>
            <li><code>new Builder\Reader($connection, $query)</code></li>
            <li><code>new Builder\Criteria($connection, $query, $type, $params)</code></li>
            <li>Eliminated SQO dependency from Builders</li>
            <li>Enhanced testability and decoupling</li>
        </ul>
    </li>
</ul>

<h4>Framework Conventions Centralized</h4>
<ul>
    <li><b>Connection.quoteColumn()</b> - SQL identifier quoting
        <ul>
            <li>Supports MySQL (<code>`</code>), PostgreSQL (<code>"</code>), SQLite (<code>"</code>), SQL Server (<code>[]</code>)</li>
            <li>Auto-detects engine and applies correct quotes</li>
            <li>Validates identifier format</li>
            <li>Handles <code>schema.table.column</code> notation</li>
        </ul>
    </li>
    <li><b>Connection.quoteCriteria()</b> - Universal SQL syntax
        <ul>
            <li>SQL Server bracket notation <code>[column]</code> as universal syntax</li>
            <li>Auto-converts to engine-specific quotes</li>
            <li>Motor-agnostic WHERE/HAVING/ORDER BY clauses</li>
            <li>Write once, run on any database engine</li>
        </ul>
    </li>
    <li><b>Connection.nullify()</b> - Empty string to NULL conversion
        <ul>
            <li>Framework philosophy: never store empty strings</li>
            <li>Automatic conversion in INSERT/UPDATE operations</li>
        </ul>
    </li>
</ul>

<h4>Enhanced Connection Class</h4>
<ul>
    <li><b>Explicit PDO Methods</b> - Better IDE autocomplete
        <ul>
            <li><code>prepare()</code>, <code>query()</code>, <code>exec()</code> explicitly defined</li>
            <li><code>inTransaction()</code>, <code>getAttribute()</code>, <code>setAttribute()</code></li>
            <li><code>lastInsertId()</code>, <code>quote()</code>, <code>errorCode()</code>, <code>errorInfo()</code></li>
            <li>Improved stack traces for debugging</li>
        </ul>
    </li>
    <li><b>SQL Server Complete Support</b>
        <ul>
            <li>Separate <code>quoteLeft</code> (<code>[</code>) and <code>quoteRight</code> (<code>]</code>)</li>
            <li>Support for dblib driver (FreeTDS)</li>
            <li>Optional port in DSN connection string</li>
        </ul>
    </li>
</ul>

<h4>Nomenclature Improvements (Internal)</h4>
<ul>
    <li><b>DDD-Aligned Terminology:</b>
        <ul>
            <li><code>Filter</code> → <code>Criteria</code> (Domain-Driven Design)</li>
            <li><code>Factory</code> → <code>Creator</code> (Specific purpose)</li>
            <li>Includes <code>Runner</code>, <code>Union</code>, <code>Resolver</code>, and <code>Pageable</code> trait</li>
        </ul>
    </li>
    <li><b>Deprecations (No Breaking Changes):</b>
        <ul>
            <li><code>SQO</code> → Use <code>Builder</code> service instead</li>
            <li><code>DBC</code> → Use <code>Connection</code> instead</li>
            <li>Old classes remain for backward compatibility</li>
        </ul>
    </li>
</ul>

<p>Public API remains unchanged. Only internal class names improved.</p>

<h3>📁 New: RESTful File Upload Support</h3>

<h4>UploadedFile Enhancements</h4>
<ul>
    <li><b>HTTP PUT/PATCH Support</b> - Full RESTful API capabilities
        <ul>
            <li>Handles <code>php://input</code> stream uploads</li>
            <li>3-level fallback system:
                <ol>
                    <li><code>move_uploaded_file()</code> (HTTP POST multipart)</li>
                    <li><code>rename()</code> (same filesystem)</li>
                    <li><code>copy() + unlink()</code> (cross-filesystem)</li>
                </ol>
            </li>
        </ul>
    </li>
    <li><b>Testing-Friendly</b> - Works with mock files</li>
    <li><b>Production-Ready</b> - Handles Docker volumes, NFS, tmpfs</li>
    <li><b>PSR-7 Compliant</b> - Full UploadedFileInterface implementation</li>
</ul>
...
<h4>Added Rules</h4>
<ul>
    <li><b>Url</b> - Validates HTTP/HTTPS URLs with <code>filter_var(FILTER_VALIDATE_URL)</code></li>
    <li><b>In</b> - Validates value exists in allowed list</li>
</ul>

<h3>🎨 Improved: Template Engine</h3>

<h4>CSRF Integration</h4>
<ul>
    <li><b>@csrf Directive</b> - Automatic CSRF token injection
        <ul>
            <li><code>&lt;meta name="csrf-token"&gt;</code> in <code>&lt;head&gt;</code></li>
            <li><code>&lt;input type="hidden"&gt;</code> in forms</li>
            <li>Auto-detects context (head vs body)</li>
            <li>Works with AJAX and traditional forms</li>
        </ul>
    </li>
</ul>
...
<h4>Lines of Code</h4>
<ul>
    <li><b>Before:</b> 8,422 LOC</li>
    <li><b>After:</b> 8,522 LOC</li>
    <li><b>Delta:</b> +100 LOC (+1.2%)</li>
</ul>
...
<ul>
    <li><b>Added:</b>
        <ul>
            <li>Security module (Cipher + CSRF + CORS)</li>
            <li>quoteCriteria() universal syntax system</li>
            <li>PUT/PATCH upload support</li>
            <li>2 validation rules</li>
        </ul>
    </li>
    <li><b>Enhanced:</b>
        <ul>
            <li>Connection class (13 explicit methods)</li>
            <li>UploadedFile (RESTful support)</li>
            <li>Builder architecture (DI pattern)</li>
            <li>Template engine (@csrf directive)</li>
        </ul>
    </li>
</ul>
...
<h4>Files</h4>
<ul>
    <li><b>Total:</b> 141 PHP files</li>
    <li><b>Modified:</b> 14 core files</li>
    <li><b>Added:</b> 5 security classes</li>
    <li><b>Breaking Changes:</b> 0 ✅</li>
</ul>

<h4>Compatibility</h4>
<ul>
    <li><b>PHP:</b> 5.4+ (maintained)</li>
    <li><b>Databases:</b> MySQL, PostgreSQL, SQLite, SQL Server</li>
    <li><b>Standards:</b> PSR-7, PSR-15</li>
    <li><b>Backward:</b> 100% compatible with v0.8</li>
</ul>

<h3>🚀 Migration Guide</h3>

<h4>From v0.8 to v0.8.1</h4>

<p><b>No breaking changes.</b> All modifications are backward compatible.</p>

<p><b>Optional Updates (Recommended):</b></p>

<ol>
    <li><b>Update Builder usage:</b>
    ...
    <li><b>Add CSRF protection:</b>
        <pre><code>'middlewares' => [
    '\Scoop\Security\Middleware\CsrfGuard'
]</code></pre>
    </li>
    <li><b>Use @csrf in templates:</b>
        <pre><code>&lt;form method="POST"&gt;
    &#64;csrf
&lt;/form&gt;</code></pre>
    </li>

    <li><b>Enable CORS if needed:</b>
        <pre><code>'middlewares' => [
